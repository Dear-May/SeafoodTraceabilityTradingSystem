<template>
  <div class="qr-container" v-loading="isLoading">
    ...
    <div v-else class="new-status-wrapper">
      <div v-if="!firstTime">
        <!-- 新的图片或提示，替换二维码 -->
        <img src="../assets/images/Wechatlogin.png" alt="微信登录" class="new-status-image"/>
...
export default {
  setup() {
    ...
    const generateQRCode = async () => {
      ...
      try {
        const response = await request.post('/api/wx/startQRCode');
    ...
    const pollQRCodeStatus = async () => {
      try {
        const response = await request.post('/api/wx/pollQRCodeStatus');
