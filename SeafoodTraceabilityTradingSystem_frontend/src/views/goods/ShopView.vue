    ...
    <div class="shop-container row">
      <div class="col-7 row">
        <div class="col-3 row">
          ...
          <div class="col-9">
            <p class="shop-name" class="fs-16">{{ shopForm.shopName }}</p>
            <p class="shop-desc" class="fs-14">{{ shopForm.shopDesc }}...</p>
          </div>
        </div>
        ...
        >
          <el-card shadow="hover" class="product-card" @click="handleGoodDetail(product.goodID)">
            ...
            <div class="product-info">
              ...
              <div class="row">
                <p class="col-6" class="info-text" v-if="product.comments>=100">
                  {{ product.comments }}+人评价</p>
                <p class="col-6" class="info-text" v-else>{{
                    product.comments
                  }}人评价</p>
              </div>
            </div>
...
async function initShopInfo() {
  try {
    const response = await request.post('/api/shop/findShopById', {
      shopId: shopid.value,
    }, {
      headers: {
...
async function initFollowShop() {
  try {
    const response = await request.post('/api/shop/findFollowShop', {
...
async function followShop() {
  try {
    const response = await request.post('/api/shop/followShop', {
      userId: UserInfoForm.value.id,
      shopId: shopid.value,
    }, {
      headers: {
...
async function unFollowShop() {
  try {
    const response = await request.post('/api/shop/unfollowShop', {
      userId: UserInfoForm.value.id,
      shopId: shopid.value,
    }, {
      headers: {
...
async function initGoodInfo() {
  try {
    const response = await request.post('/api/good/getGoodByShopId', {
      shopId: shopid.value,
    }, {
      headers: {
        'Content-Type': 'application/json;charset=UTF-8'
