          ...
          <div class="step-indicator">
            <p>
              <span
                ...
                输入用户名</span>
              <img src="../assets/images/arrow.png" alt="箭头" class="arrow-icon"/>
              <span
...
async function checkUsername() {
  ...
  if (isValidUsername(username.value)) {
    try {
      const response = await request.post('/api/user/register/check-username', {
...
async function SelectPhone(phone) {
  try {
    const response = await request.post('/api/user/register/selectPhone', {
      phone: phone
    }, {
      headers: {
        'Content-Type': 'application/json'
...
async function SelectEmail(email) {
  try {
    const response = await request.post('/api/user/register/selectEmail', {
      email: email
    }, {
      headers: {
        'Content-Type': 'application/json'
...
function completeRegistration() {
  if (password.value === confirmPassword.value && passwordStrength.value !== '弱') {
    registerForm.value.password = password.value;
    console.log(registerForm.value);
    request.post('/api/user/register', registerForm.value, {
      headers: {
        'Content-Type': 'application/json'
      },
      withCredentials: true
