...
async function initGoods() {
  ...
  try {
    const response = await request.post('/api/pay/getPayGoods', {
      token: Token.value,
    }, {
      headers: {
        'Content-Type': 'application/json;charset=UTF-8'
...
async function changeQuantity(item) {
  try {
    const response = await request.post('/api/pay/changGoodNum', {
      token: Token.value,
      specId: item.specId,
      num: item.specNumber,
    }, {
...
async function submitOrder() {
  ...
  try {
    const response = await request.post('/api/pay/submitOrder', data, {
      headers: {
        'Content-Type': 'application/json;charset=UTF-8'
      },
      withCredentials: true
