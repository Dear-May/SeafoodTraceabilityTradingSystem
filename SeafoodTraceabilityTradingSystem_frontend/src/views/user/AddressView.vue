...
import AddAddressComponent from "@/components/user/AddAddressComponent.vue"
...
async function getAddressOptions(region) {
  ...
  let isCity = false;
  try {
    const response = await request.post('/api/addresses/addressToId', {
      address: region
    }, {
      headers: {
        'Content-Type': 'application/json;charset=UTF-8'
...
async function saveAddress() {
  ...
  try {
    const response = await request.post('/api/addresses/updateAddress', {
      userId: UserInfoForm.value.id,
...
async function deleteAddress(row) {
  try {
    const response = await request.post('/api/addresses/deleteAddress', {
      addressId: row.id,
      userId: UserInfoForm.value.id,
    }, {
      headers: {
...
async function setDefault(row) {
  try {
    const response = await request.post('/api/addresses/updateDefault', {
          userId: UserInfoForm.value.id,
          addressId: row.id,
        }, {
          headers: {
