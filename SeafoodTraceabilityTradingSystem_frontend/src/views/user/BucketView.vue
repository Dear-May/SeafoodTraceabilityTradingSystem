                  ...
                  <div class="row">
                    ...
                    <div class="col-8">
                      ...
                      <div class="row">
                        ...
                        <div class="col-9" class="text-left">
      ...
      <div class="col-3">
        <div class="container cart-checkout">
          ...
          <div v-if="selectedList.length === 0">
            <img class="image" src="@/assets/images/bucket_none.png" alt="购物车图标">
...
async function initData() {
  try {
    const response = await request.post('/api/good/getCartList', {
...
async function handleQuantityChange(goodId, specId, specNumber) {
  try {
    const response = await request.post('/api/good/updateGoodCart', {
      userId: UserInfoForm.value.id,
      goodId: goodId,
      specId: specId,
      number: specNumber
...
async function getSpecList(goodId) {
  try {
    const response = await request.post('/api/good/getSpecList', {
      goodId: goodId
    }, {
      headers: {
        'Content-Type': 'application/json'
...
async function updateProductSpec(product) {
  try {
    const response = await request.post('/api/good/updateGoodCartSpec', {
      userId: UserInfoForm.value.id,
...
async function moveToFavorite(product) {
  try {
    const response = await request.post('/api/good/moveToFavorite', {
      userId: UserInfoForm.value.id,
      goodId: product.goodId,
...
async function deleteItem(product) {
  try {
    const response = await request.post('/api/good/deleteGoodCart', {
      userId: UserInfoForm.value.id,
      goodId: product.goodId,
      specId: product.specId,
    }, {
