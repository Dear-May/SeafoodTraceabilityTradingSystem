          ...
          >
            <el-sub-menu index="1">
              <template #title>
                ...
                <span class="fw-bold fs-18">账号管理</span>
              </template>
              <el-menu-item-group>
...
import AvatarChangComponent from "@/components/user/AvatarChangComponent.vue";
import {Document, Location, Setting, User} from "@element-plus/icons-vue";
import GiteeLoginComponent from "@/components/auth/GiteeLoginComponent.vue";
import GitHubLoginComponent from "@/components/auth/GitHubLoginComponent.vue";
import GoogleLoginComponent from "@/components/auth/GoogleLoginComponent.vue";
import WechatLoginComponent from "@/components/auth/WechatLoginComponent.vue";
...
async function fetchUserInfo() {
  const response = await request.post('/api/user/findUserById', {
...
async function getMsgNum_sms() {
  ...
  if (await handleLogin(password_1)) {
    ...
    try {
      const response = await request.post('/api/sms/smsXxs', {
...
async function changPhone(phone) {
  ...
  try {
    const response = await request.post('/api/user/updateUserPhone', data, {
      headers: {
        'Content-Type': 'application/json'
      },
      withCredentials: true
...
async function validateNum_email() {
  ...
  try {
    const response = await request.post('/api/email/validateNum', data, {
      headers: {
        'Content-Type': 'application/json'
      },
      withCredentials: true
...
async function changEmail(email) {
  ...
  try {
    const response = await request.post('/api/user/updateUserEmail', data, {
      headers: {
        'Content-Type': 'application/json'
      },
      withCredentials: true
...
async function completeForgetPassword() {
  if (NewPassword.value === confirmPassword.value && passwordStrength.value !== '弱') {
    ...
    try {
      const response = await request.post('/api/user/forgetPassword', registerForm.value, {
        headers: {
          'Content-Type': 'application/json'
        },
        withCredentials: true
...
async function unbindAccount(account) {
  const response = await request.post('/api/user/unbindAccount', account, {
    headers: {
      'Content-Type': 'application/json'
    },
    withCredentials: true
