  ...
  <div class="my-haichao-page">
    ...
    <div class="live-stream-container" v-loading="loading">
      ...
      <div class="header bg-light d-flex align-items-center p-3 shadow-sm row">
        ...
        <div class="col-1 row" class="text-start">
        </div>
        <div class="col-4" class="mt-15 text-end">
...
async function initLiveInfo() {
  try {
    const response = await request.post("/api/live/getLiveInfo", {
      roomId: LiveId.value,
    }, {
      headers: {
...
async function insertOnlineUser() {
  try {
    const response = await request.post('/api/live/insertOnlineUser', {
      roomId: LiveId.value,
...
async function deleteOnlineUser() {
  try {
    const response = await request.post('/api/live/deleteOnlineUser', {
      roomId: LiveId.value,
      userId: UserInfoForm.value.id,
    }, {
      headers: {
