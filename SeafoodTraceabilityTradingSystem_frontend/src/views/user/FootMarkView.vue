        ...
        <div class="product-list">
          <div v-for="(product, idx) in item.data" :key="idx" class="product-card"
            ...
            <div class="trash-icon" @click="(event) => { event.stopPropagation(); DeleteProduct(product); }">
              <img src="@/assets/images/Trash_icon.png" alt="Trash Icon" class="trash-image"/>
            </div>
          </div>
        </div>
      </div>
...
async function init() {
  try {
    const response = await request.post('/api/good/findGoodFootMark', {
...
async function DeleteProduct(product) {
  try {
    const response = await request.post('/api/good/deleteGoodFootMark', {
      userId: UserInfoForm.value.id,
