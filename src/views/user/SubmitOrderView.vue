...
const fetchQRCode = async () => {
  try {
    const response = await request.post("/api/pay/generate_qr", {
...
async function getHostIP() {
  try {
    const response = await request.get("/api/other/getHostIp");
...
async function handleMySpace(newStatus) {
  if (newStatus !== "已扫码") {
    if (newStatus === "已支付") {
      try {
        const response = await request.post("/api/pay/changOrderStatus", {
          token: token.value,
          status: newStatus,
        })
