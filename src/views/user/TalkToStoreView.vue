        ...
        <div class="chat-container d-flex">
          ...
          <div class="contact-list bg-white border-end p-2" style="width: 25%; border-radius:20px 0 0 20px;">
            <div class="search-box mb-3">
              <div style="text-align: start" class="mb-1">
                <i class="bi bi-chat-heart"></i>&nbsp;&nbsp;<a class="fw-bold">聊天列表</a>
              </div>
              <input type="text" class="form-control" placeholder="搜索最近联系人..." class="rounded-20">
            </div>
            ...
            <div v-if="selectedContactIndex !== null" class="chat-section flex-grow-1 p-2">
              ...
              <el-footer>
                ...
                <div style="text-align: right;">
                  <el-button type="primary" @click="send" class="rounded-20">发送</el-button>
                </div>
              </el-footer>
            </div>
          </el-container>
          ...
          <div class="additional-info bg-white border-start p-2" v-if="selectedContactIndex !== null"
            ...
            <div v-for="product in products" :key="product.id" class="product-item d-flex align-items-center mb-2">
              ...
              <button class="btn btn-outline-primary btn-sm" class="rounded-20"
...
async function initContacts() {
  try {
    const response = await request.post('/api/chat/getChatSession', {
...
async function IsInChatSession() {
  ...
  if (shopId) {
    ...
    if (!contactExists.value) {
      try {
        const response = await request.post('/api/chat/createChatSession', {
          userId: UserInfoForm.value.id,
...
async function selectContact(index) {
  ...
  try {
    const response = await request.post('/api/chat/getChatDetail', {
      userId: UserInfoForm.value.id,
...
async function deleteContact(index) {
  ...
  try {
    const response = await request.post('/api/chat/deleteChatSession', {
      userId: UserInfoForm.value.id,
      shopId: contactId
    }, {
      headers: {
...
async function uploadImage(file) {
  try {
    ...
    const response = await request.post('/api/chat/uploadImage', formData, {
      headers: {
...
async function send() {
  ...
  try {
    const response = await request.post('/api/chat/sendMessage', {
      userId: UserInfoForm.value.id,
...
async function getProducts() {
  try {
    const response = await request.post('/api/pay/chat/getChatOrderList', {
      userId: UserInfoForm.value.id,
      shopId: selectedContact.value.id
    }, {
      headers: {
...
async function sendProductLink(product) {
  ...
  try {
    const response = await request.post('/api/chat/sendOrderLink', {
      userId: UserInfoForm.value.id,
      shopId: selectedContact.value.id,
