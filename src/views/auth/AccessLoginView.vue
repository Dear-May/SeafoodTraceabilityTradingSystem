<template xmlns="http://www.w3.org/1999/xhtml">
  <el-container class="band-container" :class="{ 'night-mode': isNightMode }">
    <el-header class="band-logo header-left">
      <img src="../assets/images/logo.png" alt="Logo" class="band-img" @click="handleHome"/>
...
const bindAccount = async () => {
  try {
    const response = await request.post('/api/user/bindAccount', bindForm.value, {
      headers: {
        'Content-Type': 'application/json'
      },
      withCredentials: true
...
onMounted(async () => {
  try {
    ...
    if (token) {
      ...
      const response = await request.post('/api/callback/ThirdLogin/userInfo', {}, {withCredentials: true});
