<template xmlns="http://www.w3.org/1999/xhtml">
  <el-container class="login-container" :class="{ 'night-mode': isNightMode }">
    <el-header class="login-logo header-left">
      <img src="../assets/images/logo.png" alt="Logo" class="logo-img" @click="handleHome"/>
      ...
      <div class="login-section" style="display: flex; width: 100%; align-items: center;">
        <div class="qr-login-section" style="margin-top: 50px; border-right: 1px solid #ddd; padding-right: 40px;">
          <p class="qr-login-text" style="margin-bottom: 20px; margin-top: 0; align-self: center; font-size: 24px;">
            手机扫码登录</p>
          <div class="qr-login">
              <OAuthLoginButton platform="gitee" displayName="Gitee" iconSrc="@/assets/images/mayun.png" />
  <OAuthLoginButton platform="github" displayName="GitHub" iconSrc="@/assets/images/GitHub.png" />
  <OAuthLoginButton platform="google" displayName="Google" iconSrc="@/assets/images/google.png" />
<wxLogin/>
...
import GiteeLogin from "@/components/auth/GiteeLoginComponent.vue";
import GitHubLogin from "@/components/auth/GitHubLoginComponent.vue";
import wxLogin from "@/components/auth/WechatLoginComponent.vue"
import GoogleLogin from "@/components/auth/GoogleLoginComponent.vue";
...
async function getUserBaseInfo(username) {
  try {
    const response = await request.post('/api/user/getUserBaseInfo', {
      username: username
    }, {
      headers: {
